...
//	}
	...
	public static void updateCheck(Activity mAc) {


		RetrofitFactory.getInstence().API().uploadAPP().compose(TransformerUi.setThread()).subscribe(new BaseObserver<UploadAPPBean>(mAc) {
			@Override
			protected void onSuccess(BaseEntity<UploadAPPBean> t) throws Exception {
				...
				if (uploadAPPBean.isNeedUpdate()) {
					Intent intent = new Intent(mAc, DownloadService.class);
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
						mAc.startForegroundService(intent);
					} else {
						mAc.startService(intent);
					}
					//绑定服务
					mAc.bindService(intent, connection, BIND_AUTO_CREATE);

					dialog = new UploadAPPDialog(mAc, R.style.dialog_center);
