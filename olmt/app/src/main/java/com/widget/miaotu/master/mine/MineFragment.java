        ...
        switch (view.getId()) {
            case R.id.rel_me_release://我的发布
                if (!isLogin()) {
                    ...
                    userIntent.setClass(getActivity(), SupplyAndAskToBuyActivity.class);
            ...
            case R.id.rel_me_business_management://企业管理
                if (!isLogin()) {
                    ...
                    if ("1" != SPStaticUtils.getString(SPConstant.ISCOMPANY)) {
                        ...
                        intent4.putExtra("id", "");
                        intent4.setClass(getFragmentContext(), CompanyDetailActivity.class);
                        startActivity(intent4);
                    } else {
                        startActivity(new Intent(getFragmentContext(), CompanyInfoAddActivity.class));
                    }
                }
        ...
        RetrofitFactory.getInstence().API().getUserInfo(new TokenBeanNew(""))
                ...
                .subscribe(new BaseObserver<SMSLoginBeanNew>(getActivity()) {
                    ...
                    protected void onSuccess(BaseEntity<SMSLoginBeanNew> t) throws Exception {
                        ...
                        //登陆后将用户的id上报bugly
                        CrashReport.setUserId(mUserId);  //该用户本次启动后的异常日志用户ID都将是9527
                        //最后统一服务后台配
