...
public class YanZhengTixingMessageAdapter extends BaseQuickAdapter<MsgVerifyBean, BaseViewHolder> {
    ...
    public YanZhengTixingMessageAdapter(Context context, ItemClickCallBack itemClickCallBack) {
    ...
    protected void convert(BaseViewHolder helper, MsgVerifyBean item) {
        ...
        helper.getView(R.id.stv_yztx_detail_tongguo).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (mItemClickCallBack != null) {
                    mItemClickCallBack.onTongGuoClick(item);
                }
            }
        });
        helper.getView(R.id.stv_yztx_detail_hulue).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mItemClickCallBack != null) {
                    mItemClickCallBack.onHuLueClick(item);
                }
            }
        });
    }
    ...
    public interface ItemClickCallBack {
        void onTongGuoClick(MsgVerifyBean item);
        void onHuLueClick(MsgVerifyBean item);
    }
}
