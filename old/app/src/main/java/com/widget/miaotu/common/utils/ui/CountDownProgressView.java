...
public class CountDownProgressView extends AppCompatTextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null) {
            ...
            // 进度条颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_progressColor2)) {
                //有这个属性索引 获取用户设置的颜色
                progressColor = typedArray.getColor(R.styleable.CountDownProgress_progressColor2, 0);
            } else {
                //没有这个属性索引 默认是蓝色
                progressColor = typedArray.getColor(R.styleable.CountDownProgress_progressColor2, Color.parseColor("#0000FF"));
            }
